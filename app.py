...
def ws_listener(q, status_holder):
    async def listen():
        try:
            ...
            status_holder["status"] = "error: timed out waiting for data"
...
try:
    ...
    stats = {"running": False, "total_processed": 0, "total_flagged": 0, "loss_prevented": 0.0, "threshold": 0.55}
...
st.caption(f"Messages in queue buffer: {len(st.session_state.transactions)}")
...
if st.session_state.transactions:
    ...
    if not anomalies.empty:
        ...
        st.info("No anomalies flagged yet in this session.")
else:
